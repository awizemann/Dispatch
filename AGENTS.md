...
- Long-form documents (guides, research, specs) → the wiki at write time:
  `write_memory(project: "dispatch-wiki", …)`; keep the distilled facts in a memory note with
  a `documented_in` relation. `status:` only marks retirement
  (`deprecated`/`superseded`/`historical`/`resolved`); current facts carry none.

**2. Wiki (`wiki/`) — long-form reference.** Search on demand (`project: "dispatch-wiki"`);
...
(`project: "dispatch-design"`). Reference only: prototype code under `design/` is never
...
"dispatch-code"`. Grep is the fallback for unindexed languages.
...
`search_memories(project: "dispatch-templates")`, read the `manifest.md` + its `reference/`
