...
if [ -d "$ROOT/wiki" ]; then
  echo ''
  echo 'Wiki present (wiki/): search the `dispatch-wiki` project via the memophant MCP server, or grep wiki/.'
fi
if [ -d "$ROOT/design" ]; then
  echo ''
  echo 'Design tier present (design/): consult before UI work — search the `dispatch-design` project via the memophant MCP server, or grep design/.'
fi
