#!/usr/bin/env bash
#
# Dispatch landing-site helper — builds the marketing landing page from
...
#
# Bootstrap (one-time, NOT done by this script): a .gh-pages-worktree/ clone of
...
#   {{VERSION}}   — current Dispatch version (read from Dispatch.xcodeproj/project.pbxproj MARKETING_VERSION)
...
PBXPROJ="$REPO_ROOT/Dispatch.xcodeproj/project.pbxproj"
PY="${PYTHON:-python3}"
...
need_ghpages() {
  ...
  Bootstrap first (see the header comment in this script), then:
  git worktree add .gh-pages-worktree gh-pages"
}

# ---------- token resolvers ----------

# Pull MARKETING_VERSION from the Xcode project. Dispatch has no project.yml
# (the .xcodeproj is committed directly, no xcodegen) — the pbxproj IS the
# source of truth for the in-development version.
resolve_version() {
  if [[ -f "$PBXPROJ" ]]; then
    "$PY" -c '
import re
src = open("'"$PBXPROJ"'", "r", encoding="utf-8").read()
m = re.search(r"MARKETING_VERSION\s*=\s*([0-9][0-9.A-Za-z\-]*)\s*;", src)
...
cmd_publish() {
  ...
  log "Published. Verify at https://awizemann.github.io/Dispatch/"
}
